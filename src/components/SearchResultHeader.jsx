...
const SearchResultHeader = () => {
  ...
  useEffect(() => {
    // return statement work as unmount, while going back, it will set the setImageSearch as false
    return () => setImageSearch(false);
  }, []);
  // empty dependency array works on load
  ...
  return (
    <div className="p-[15px] pb-0 md:pr-5 md:pl-20 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white">
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center grow md:translate-x-[-20px]">
          <Link to="/">
            <img
              className="hidden md:block w-[92px] mr-5 lg:mr-10"
              src={Logo}
            />
          </Link>
      ...
      <div className="flex ml-0 xss:ml-[-30px] mt-3">
        {menu.map((menu, index) => (
          <span
            key={index}
